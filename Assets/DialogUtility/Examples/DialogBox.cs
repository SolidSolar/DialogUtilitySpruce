...
public class DialogBox : MonoBehaviour
{
    ...
    [SerializeField]
    private DialogGraphContainer _currentDialog;
    ...
    private IEnumerator ShowMessage(string message)
    {
        ...
        for (int i = 0; i < u; i++)
        {
            if(st>=message.Length)
                break;
            string endStr = st + charactersPerMessage >= message.Length ? message.Substring(st) : message.Substring(st, charactersPerMessage);
